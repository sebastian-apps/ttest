...
var welchs = {
  ...
  getFalseDiscoveryRate(FP, TP) {
    return math.round(100*Number(FP)/(Number(FP) + Number(TP)),2);
  },

  t_Distrib(t, dof){
    // For a given x, return the t distribution y value.
    let y = (math.gamma((dof+1)/2)/(math.sqrt(dof*math.pi)*math.gamma(dof/2))) * (1+((t**2)/dof))**(-(dof+1)/2);
    return math.round(y, 6);
  },
  ...
  getAlphaCurve(group1, crit_t_value){  
    ...
    let alpha_curve = [];
    // this datapoint connects with the critical t vertical line
    alpha_curve.push({x: crit_t_value, y: this.t_Distrib(crit_t_value, group1.df)});
    for(var i = 0; i < group1.dist.length; i++) {
      if (group1.dist[i]['x'] > crit_t_value){
        alpha_curve.push({x: group1.dist[i]['x'], y: group1.dist[i]['y']});
      }
  ...
  getBetaCurve (group2, crit_t_value, ncp){  
    ...
    // this datapoint connects with the critical t vertical line
    beta_curve.push({x: crit_t_value, y: this.t_Distrib(crit_t_value-ncp, group2.df)}); // ncp acts as offset
    return beta_curve;
  },


  getAlpha(crit_t_value, df){
    return 1 - this.computeArea(crit_t_value, df);
  },

  getBeta(crit_t_value_beta, df){
    return this.computeArea(crit_t_value_beta, df);
  },
